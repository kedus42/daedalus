    ...
    private:
        ...
        void extension_callback_(const daedalus::ExtensionConstPtr &extension_cmd){
            if(extension_cmd->side == 0 ){
                int i =0;
                int compensated_pos = extension_cmd->index;
                if (compensated_pos < 0)
                    compensated_pos += 7;
                while(i<3){
                    left_array_.data[compensated_pos*3 + i] = extension_cmd->ext*0.33333333333333333;
                    i++; 
                }
                ...
                if (compensated_pos < 0)
                    compensated_pos += 7;
                while(i<3){
                    right_array_.data[compensated_pos*3 + i] = extension_cmd->ext*0.33333333333333333;
                    i++; 
                }
        ...
        void pos_update(const sensor_msgs::JointStateConstPtr &state){
            ...
            left_drive_pos_ = l_temp / (PIl4);
            right_drive_pos_ = r_temp / (PIl4);
        }
    public:
        Allocator(){
            ...
            left_array_.layout.dim.push_back(std_msgs::MultiArrayDimension());
            right_array_.layout.dim.push_back(std_msgs::MultiArrayDimension());
            left_array_.layout.dim[0].size=24;
            right_array_.layout.dim[0].size=24;
            int i = 0;
            while (i<24){
